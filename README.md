...
`/diff remote` accepts GitHub, Graphite, and Meteorite pull-request URLs, `owner/repo#number`, or a remote branch.

GitHub and GitStream reviews can be submitted from the UI as:
...
`DISCUSS` items never post remotely. Self-approval is refused. GitStream verifies the reviewed base and head immediately before its atomic submission. If a remote target changes during an agent discussion, the old line mappings are not reused.
...
- `submit_pr_review` submits a confirmed GitHub or GitStream review.
