...
`/diff`, `/code`, and `/code-diff` open an interactive code diff editor for Pi with comments, editable line suggestions, AI-assisted review handoff, GitHub and GitStream PR submission support, and local or remote diff review.

It is inspired by Mario Zechner's [pi-diff-review](https://github.com/badlogic/pi-diff-review).

It lets you stop after an agent turn, walk a local diff or remote PR inside Pi, add fast line/file/all-lines annotations, and route the result either back to the agent or into a GitHub or GitStream PR review.
...
- remote branches
- GitHub, Graphite, or Meteorite PR URLs
...
- mark feedback as either:
  ...
  - `COMMENT` — a real pull-request comment when the review is a GitHub or GitStream PR; for a local review it becomes a local comment to the agent about the change
...
- for GitHub and GitStream PR reviews, run a grammar and semantic-safety pass with the active Pi model, automatically apply safe corrections, and submit the confirmed verdict directly
...
- `/diff remote <url | branch>` reviews a remote branch or PR. It accepts a remote branch, a GitHub or Graphite PR URL, a Meteorite URL, or `owner/repo#123`.
...
- `/diff remote <branch | url>` or `open_code_diff({ args: "remote <branch | url>" })` accepts a plain remote branch, a GitHub PR URL, a Graphite PR URL, a Meteorite URL, or `owner/repo#number`
- Meteorite accepts both `https://meteorite.shopify.io/repos/owner/repo/pulls/123` and the same URL ending in `/files`; both forms share the same saved-review identity
...
When the review is a GitHub or GitStream PR, finishing the review opens an end-action menu:
...
6. `COMMENT` line items are posted as inline review comments. `MODIFY` line edits are posted as inline comments containing a suggested diff. On GitHub, `COMMENT` file/all-lines items become the review body. On GitStream, they use the native file-comment anchor. `DISCUSS` items are never submitted.
7. Approval refuses self-approval with a clear message. GitHub `request changes` requires a body or at least one inline comment; GitStream requires a review body.
8. GitStream metadata, identity, and submission use `gs api`. Immediately before posting, pi-code-diff verifies that the live base and head SHAs still match the reviewed target. The verdict, body, line comments, and file comments are then sent in one atomic review request. Malformed output, authentication errors, target drift, unsupported comment locations, and command failures stop without a fallback post.
