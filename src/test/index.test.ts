...
describe("code diff extension", () => {
  ...
  it("retains stale fields after a successful mixed GitStream submission", async () => {
    const tools = new Map<string, any>();
    const file = remoteReviewFile();
    const mapped = { id: "gitstream-mapped", fileId: file.id, scope: "all-files" as const, side: "added" as const, intent: "comment" as const, startLine: 4, endLine: 4, body: "Mapped GitStream note", anchorStatus: "mapped" as const };
    const stale = { id: "gitstream-stale", fileId: file.id, scope: "all-files" as const, side: "deleted" as const, intent: "modify" as const, startLine: 8, endLine: 9, body: "replacement", originalText: "original\r\nline", captureHash: { algorithm: "sha256" as const, value: "5".repeat(64) }, anchorStatus: "stale" as const };
    const payload = { type: "submit" as const, allComment: "GitStream body", allIntent: "comment" as const, comments: [mapped] };
    const session = reviewSessionData({ allComment: payload.allComment, allIntent: payload.allIntent, comments: [mapped, stale] });
    mocks.resolveRemoteReviewTarget.mockResolvedValue({
      ...remoteTarget(),
      provider: "gitstream",
      repo: "shop/world",
      remote: "https://meteorite.shopify.io/repos/shop/world/pulls/42",
      pullRequest: { ...remoteTarget().pullRequest, number: "42", repo: "shop/world", baseRefOid: "base-sha", headRefOid: "head-sha" },
    });
    mocks.getReviewWindowDataForRevisionRange.mockResolvedValue({ repoRoot: "/repo", files: [file], branchBaseRevision: "origin/main", modifiedRevision: "origin/gitstream/42/head", visibleScopes: ["all-files"] });
    mocks.runReviewApp.mockImplementation(async (_ctx, options) => {
      options.onSessionChange(session);
      return payload;
    });
    ...
    await tools.get("open_code_diff").execute("tool-call", { args: "remote shop/world#42" }, new AbortController().signal, vi.fn(), ctx);
    ...
    expect(mocks.saveReviewSessionWithStatus).toHaveBeenLastCalledWith(
      "/repo|origin/main|head-sha|https://meteorite.shopify.io/repos/shop/world/pulls/42",
      expect.objectContaining({ state: expect.objectContaining({ draft: { allComment: "", allIntent: "comment", comments: [stale] } }) }),
      "automatic-session",
    );
    expect(mocks.deleteReviewSession).not.toHaveBeenCalled();
  ...
  it("runs a literal Meteorite /diff remote target through the normal confirmed review flow", async () => {
    const commands = new Map<string, { handler: (args: string, ctx: any) => Promise<void> }>();
    const file = remoteReviewFile();
    const meteoriteUrl = "https://meteorite.shopify.io/repos/shop/world/pulls/2002491/files";
    mocks.resolveRemoteReviewTarget.mockResolvedValue({
      ...remoteTarget(),
      provider: "gitstream",
      repo: "shop/world",
    ...
    mocks.getReviewWindowDataForRevisionRange.mockResolvedValue({
      ...
      files: [file],
      branchBaseRevision: "origin/main",
      modifiedRevision: "origin/gitstream/2002491/head",
      visibleScopes: ["all-files"],
    });
    ...
    codeDiffExtension(pi as never);
    await commands.get("diff")!.handler(`remote ${meteoriteUrl}`, ctx);
    await vi.waitFor(() => expect(mocks.submitPullRequestReview).toHaveBeenCalled());

    expect(mocks.resolveRemoteReviewTarget).toHaveBeenCalledWith(pi, "/repo", meteoriteUrl, undefined, expect.any(Function));
    ...
    const prUrl = "https://meteorite.shopify.io/repos/shop/world/pulls/2002491";
