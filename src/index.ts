...
import { buildGitstreamComments, buildInlineComments, buildReviewBody, submitPullRequestReview, type ReviewInlineComment, type ReviewVerdict } from "./review-submit.js";
...
function pullRequestProviderName(target: RemoteReviewTarget): string {
  return target.provider === "gitstream" ? "GitStream" : "GitHub";
}
...
export default function codeDiffExtension(pi: ExtensionAPI, options: { runExternalEditor?: ExternalEditorLauncher } = {}) {
  ...
  async function finishRemotePrReview(ctx: ExtensionContext, files: Parameters<typeof composeReviewPrompt>[0], result: ReviewSubmitPayload, target: RemoteReviewTarget): Promise<RemotePrFinishResult> {
    const pr = target.pullRequest!;
    const inlineComments = target.provider === "gitstream"
      ? buildGitstreamComments(files, result.comments)
    ...
    const includeFileComments = target.provider !== "gitstream";
  ...
  pi.registerTool({
    ...
    description: "Submit a GitHub or GitStream pull request review after confirmation. Refuses self-approval and validates immutable GitStream targets before one atomic submission.",
    promptSnippet: "Submit a confirmed GitHub or GitStream PR review verdict through the matching provider.",
    promptGuidelines: [
      ...
      "For GitStream, pass provider=gitstream and baseCommitId so target drift can be rejected before submission.",
      "Pass cwd to the local checkout of the repository so the provider CLI runs in the right place.",
    ],
    parameters: Type.Object({
      provider: Type.Optional(Type.Union([Type.Literal("github"), Type.Literal("gitstream")], { description: "Pull request provider; defaults to github" })),
      ...
      baseCommitId: Type.Optional(Type.String({ description: "Immutable PR base SHA, required for GitStream" })),
      verdict: Type.Union([
      ...
      }), { description: "Line review comments, or GitStream file comments with subject_type=file" })),
