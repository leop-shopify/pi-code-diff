...
export default function codeDiffExtension(pi: ExtensionAPI) {
  ...
  async function finishRemotePrReview(ctx: ExtensionContext, files: Parameters<typeof composeReviewPrompt>[0], result: ReviewSubmitPayload, target: RemoteReviewTarget): Promise<ReviewRunStatus> {
    ...
    const confirmed = await ctx.ui.confirm(
      ...
      "The agent will grammar-fix the text, confirm with you, then post via provider-cli api. Nothing is posted yet.",
    );
    if (!confirmed) {
  ...
  async function openDiffMenu(ctx: ExtensionContext): Promise<ReviewRunStatus> {
    ...
    if (choice === remoteChoice) {
      const value = await ctx.ui.input("Remote branch, GitHub/stack-host PR URL, or owner/repo#number");
  ...
  pi.registerTool({
    ...
    description: "Launch the pi-code-diff interactive review TUI for reviewing git diffs. Supports local diffs, custom ref ranges, remote branches, and same-repo GitHub or stack-host PR URLs.",
  ...
  pi.registerTool({
    ...
    description: "Submit a GitHub pull request review (approve, request changes, or comment) via provider-cli api. Refuses to approve a PR authored by the current user.",
